...
assessment_pipeline=issues = jira.search_issues ('project = MCOE AND issuetype = Project AND Product in (vRA,VCF, "NSX - Standalone migration") AND status in (Approved, Backlog, "SOW Prep", "SOW Approval & Signature") AND key not in ("MCOE-141","MCOE-140") AND created >= startOfWeek()',maxResults=1000)
